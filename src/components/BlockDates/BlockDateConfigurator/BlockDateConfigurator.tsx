...
import { enabledDays } from '../BlockDatesMock'

export const BlockDateConfigurator = () => {
  ...
  return (
    <div>
      <h2>Configurar Bloqueo de Fechas</h2>
      <div>
        <DatePicker
          selected={startDate}
      ...
      <ul>
        {blockedDates.filter(range => range.status === "disabled").map((range, index) => (
          <li key={index}>
